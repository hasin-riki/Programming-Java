...
columnNames = ["Iteration Count", "Root Approximation", "Function Value at Root Approximation"]
iterationCount = []
roots = []
functionVals = []
...
def bisectionMethod():
    ...
    file.write("\n\n")
    file.write("{:<30}{:<30}{:<30}".format(columnNames[0],columnNames[1],columnNames[2]))
    file.write("\n")
    
    for val in range(len(iterationCount)):
        file.write("{:<30}{:<30}{:<30}".format(iterationCount[val],roots[val],functionVals[val]))
        file.write("\n")
